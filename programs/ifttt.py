...
def run(settings, message):
    ...
    if keyword:
        ...
        url = 'https://maker.ifttt.com/trigger/{}/with/key/{}'.format(action['action'], IFTTT_KEY)
        receive = requests.get(url)
        message_out = action['reply']

    else:
        message_out = 'Keyword %s not understood. Available keywords: %s'%(keyword, ', '.join(keywords))
